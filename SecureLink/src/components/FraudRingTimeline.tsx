// Progress checkpoint: edited 2026-02-10 — fixed memory leak and crash on timeline
import { memo, useMemo } from 'react';
import { motion } from 'framer-motion';
import { Clock, AlertCircle } from 'lucide-react';
import { FraudRing, Transaction } from '../types';

interface FraudRingTimelineProps {
  fraudRings: FraudRing[];
  allTransactions: Transaction[];
}

export const FraudRingTimeline = memo(function FraudRingTimeline({ fraudRings, allTransactions }: FraudRingTimelineProps) {
  ...
  const ringTransactions = useMemo(() => {
    ...
    fraudRings.forEach(ring => {
      const txs = allTransactions
        .filter(tx => tx.jlynFingerprint === ring.fingerprint)
  ...
  return (
    <div className="w-full h-full rounded-xl bg-gradient-to-br from-slate-900/70 to-slate-800/50 backdrop-blur-xl border border-indigo-500/20 p-5 flex flex-col">
      <div className="mb-4">
        ...
        <p className="text-xs text-gray-400">{fraudRings.length} detected rings</p>
      </div>
